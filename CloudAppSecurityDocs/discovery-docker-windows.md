...
1. 展開の詳細が表示されます。 ダイアログから実行コマンドを**コピー**します。 [クリップボードにコピー] アイコン、[ ![ クリップボードにコピー] アイコンを使用でき ](media/copy-icon.png) ます。 この情報は後で必要になります。
...
1. 次のコマンドを実行して、Windows Docker installer PowerShell スクリプトファイルをダウンロードします。`Invoke-WebRequest https://adaprodconsole.blob.core.windows.net/public-files/LogCollectorInstaller.ps1 -OutFile (Join-Path $Env:Temp LogCollectorInstaller.ps1)`

    インストーラーが Microsoft によって署名されていることを確認するには、「[インストーラー署名の検証](#validate-signature)」を参照してください。

1. PowerShell スクリプトの実行を有効にするには、を実行します。`Set-ExecutionPolicy RemoteSigned`

1. を実行 `& (Join-Path $Env:Temp LogCollectorInstaller.ps1)` します。これにより、Docker クライアントがコンピューターにインストールされます。 ログ コレクター コンテナーのインストール中に、コンピューターが 2 回再起動され、もう一度ログインする必要があります。 **Docker クライアントが Linux コンテナーを使用するように設定されていることを確認します。**
...
### <a name="optional---validate-installer-signature"></a>省略可能: インストーラーの署名の検証 <a name="validate-signature"></a>
